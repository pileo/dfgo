...
func (s *Session) Run(ctx context.Context, input string) Result {
	...
	for {
		...
		// If no tool calls, the agent is done (natural exit).
		...
		for _, tc := range toolCalls {
			...
			// Loop detection.
			if s.detector.Record(tc.Name, string(tc.Arguments)) {
...
func (s *Session) buildRequest() llm.Request {
	...
	if s.cfg.ProjectDoc != "" {
		pb.WithProjectDoc(s.cfg.ProjectDoc)
	}
	if s.cfg.UserPrompt != "" {
